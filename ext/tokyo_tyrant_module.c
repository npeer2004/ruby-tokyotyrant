...
static VALUE mTokyoTyrant_close(VALUE vself)
{
    ...
    if (!tcrdbclose(db))
        mTokyoTyrant_exception(vself, "close error: %s");
    return Qtrue;
}
...
static VALUE mTokyoTyrant_misc(int argc, VALUE *argv, VALUE vself)
{
    ...
    TCLIST *list, *args;
    TCRDB *db = mTokyoTyrant_getdb(vself);
    rb_scan_args(argc, argv, "12", &vname, &vopts, &vargs);
    if (vopts == Qnil)
        vopts = INT2NUM(0);
    if (vargs == Qnil)
        vargs = rb_ary_new();
